...
void drawLegend(const GfxRenderer& renderer, const Rect& rect) {
  ...
  for (int index = 0; index < LEVEL_COUNT; ++index) {
    ...
    // Vertically center the label glyph against the swatch so text and box sit level.
    const int labelY = swatch.y + (LEGEND_SWATCH_SIZE - renderer.getTextHeight(SMALL_FONT_ID)) / 2;
    renderer.drawText(SMALL_FONT_ID, itemX + 28, labelY, LEVELS[index].label);
  }
}
}  // namespace
...
void ReadingHeatmapActivity::loop() {
  // Exit on the Back *release* edge (like the menu and sibling screens). Using the press
  // edge here let the same Back actuation pop to the menu and then trigger the menu's own
  // release handler, which dropped the user all the way out to the Apps list.
