...
void FlashcardReviewActivity::onEnter() {
  ...
  reviewQueue = deck.buildReviewQueue(today, g_flashcardConfig.newPerDay, g_flashcardConfig.maxReview);
  currentQueueIndex = 0;
  cardState = CardState::Front;
  saveCounter = 0;
  if (reviewQueue.empty()) {
    startActivityForResult(std::make_unique<FlashcardDoneActivity>(renderer, mappedInput),
...
void FlashcardReviewActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, deck.getName().c_str());
...
void FlashcardReviewActivity::renderFront(const FlashcardCard& card) {
  ...
  int mainLineHeight = renderer.getLineHeight(NOTOSERIF_14_FONT_ID);
  ...
  while (pos < mainText.size()) {
    ...
    std::string line = (nl == std::string::npos) ? mainText.substr(pos) : mainText.substr(pos, nl - pos);
    renderer.drawCenteredText(NOTOSERIF_14_FONT_ID, textY, line.c_str());
...
void FlashcardReviewActivity::renderBack(const FlashcardCard& card) {
  ...
  const char* ratings_raw[] = {tr(STR_FLASHCARD_AGAIN), tr(STR_FLASHCARD_EASY), tr(STR_FLASHCARD_HARD),
                               tr(STR_FLASHCARD_GOOD)};

  const char* intervals[4];
  const char* ratings[4];
  if (origOrientation == GfxRenderer::Orientation::PortraitInverted) {
    intervals[0] = intervals_raw[3];
    intervals[1] = intervals_raw[2];
    intervals[2] = intervals_raw[1];
    intervals[3] = intervals_raw[0];
    ratings[0] = ratings_raw[3];
    ratings[1] = ratings_raw[2];
    ratings[2] = ratings_raw[1];
    ratings[3] = ratings_raw[0];
  } else {
    for (int i = 0; i < 4; i++) {
      intervals[i] = intervals_raw[i];
      ratings[i] = ratings_raw[i];
    }
  }
  ...
  // Separator line
  textY += metrics.verticalSpacing;
  renderer.drawLine(metrics.contentSidePadding, textY, pageWidth - metrics.contentSidePadding, textY, true);
  textY += metrics.verticalSpacing;
