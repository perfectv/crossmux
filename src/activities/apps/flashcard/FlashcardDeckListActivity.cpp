...
void FlashcardDeckListActivity::loop() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    ...
    if (selectedIndex < deckCount) {
      ...
      startActivityForResult(std::make_unique<FlashcardFilePickerActivity>(renderer, mappedInput),
...
void FlashcardDeckListActivity::render(RenderLock&&) {
  ...
  // Header
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_FLASHCARD_DECK_SELECT));

  int y = metrics.topPadding + metrics.headerHeight + metrics.verticalSpacing;

  // Subtitle: "今日复习: 25/250"
  char subtitle[64];
  snprintf(subtitle, sizeof(subtitle), "%s %d/%d", tr(STR_FLASHCARD_REVIEW_TODAY), static_cast<int>(totalDueToday),
  ...
  if (y + smallItemHeight <= maxY) {
    ...
    renderer.drawText(UI_10_FONT_ID, sidePad + 8, y + 5, tr(STR_FLASHCARD_IMPORT), selectedIndex != importIdx);
  ...
  if (y + smallItemHeight <= maxY) {
    ...
    renderer.drawText(UI_10_FONT_ID, sidePad + 8, y + 10, tr(STR_FLASHCARD_SETTINGS), selectedIndex != settingsIdx);
    // 最后一项不加间距
  }
