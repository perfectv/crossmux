...
void FlashcardSettingsActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_FLASHCARD_SETTINGS));
  ...
  GUI.drawList(
      renderer, Rect{0, menuTop, pageWidth, menuHeight}, SETTINGS_COUNT, selectedIndex,
      [](int i) -> std::string {
        if (i == 0) return tr(STR_FLASHCARD_NEW_PER_DAY);
        return tr(STR_FLASHCARD_MAX_REVIEW);
      },
      nullptr, nullptr, [&values](int i) -> std::string { return values[i]; }, true);
