...
uint32_t countCurrentBookmarksFromStats() {
  // This fork has no bookmark feature (no BookmarkStore), so bookmark-based
  // achievements can never make progress. Keep the metric at 0 rather than
  // pulling in a bookmark subsystem that does not exist here.
  return 0;
}
...
std::vector<AchievementView> AchievementsStore::buildViews() const {
  ...
  for (const auto& definition : definitions()) {
    // This fork has no bookmark feature, so bookmark achievements can never be
    // earned — hide them from the list rather than showing dead 0/N entries.
    if (definition.metric == AchievementMetric::TotalBookmarksAdded) {
      continue;
    }
    uint64_t currentValue = 0;
    switch (definition.metric) {
